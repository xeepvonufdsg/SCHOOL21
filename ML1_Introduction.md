...
![day_1_scheme](assets/img_1935.png)
...
The first step of capturing patterns from *train* data is called **fitting** or **training** the model. The data used to fit the model is called the **training data**. All the characteristics that you base your decision on are called **features**. For example, the number of bedrooms or bathrooms are features. And the price is the **target**. Your simple algorithm in the above case might be: "If a house has more than 2 bedrooms, then its price is 188000,otherwiseitspriceis188000,otherwiseitspriceis188000, otherwise its price is 178000".
...
**Classification**. In classification problems, the output space is a set of *C* unordered and mutually exclusive labels known as **classes**, Y=1,2,...,CY=1,2,...,CY = {1,2,...,C}. The problem of predicting the class label given an input is also called **pattern recognition**. (If there are just two classes, often denoted by y∈{0,1}y∈{0,1}y\in\{0,1\} or y∈{−1,+1}y∈{−1,+1}y\in\{−1, +1\}, it is called *binary classification*.) A classification problem with multiple classes (greater than 2) is called *multiclass.* There are also a variety of multilabel problems.

For example, we use *binary classification* to answer the question: does the patient have heart disease? *Multiclass classification* is used in cases where each sample is assigned to one and only one label: a fruit can be either an apple or a pear or a banana, but not both at the same time.

**Regression**. Suppose we want to predict a real-valued quantity y∈Ry∈Ry ∈ R instead of a class label y∈{1,...,C}y∈{1,...,C}y \in\{1,...,C\}; this is known as regression. See the house price prediction example above.
...
In supervised learning, we assume that each input example *x* in the training set has an associated set of output targets *y*, and our goal is to learn the input-output mapping.

A much more interesting task is to try to "make sense" of the data, as opposed to just learning a mapping. That is, we just get observed **inputs** D={xi:i∈{1,…,N}D={xi:i∈{1,…,N}D = \{x\_i:i \in \{1, \dots, N\} without any corresponding **outputs** y\_i∗∗y\_i∗∗y\\_i \*\*\*\*. This is called unsupervised learning. In other words, unsupervised learning is when you have only input data (X) and no corresponding output variables.
...
2. Introduction to Data Analysis
   ...
   6. We'll work with only 3 features: 'bathrooms', 'bedrooms', 'interest\_level' and with the target column 'price'. Create a dataframe with only these columns.
3. Statistical Data Analysis
   ...
   4. Characteristics Analysis
      1. What is the type of column 'interest\_level'?
...
4. Creating Features
   1. This step is very broad. You can create as many features as you want. For example, you can add 3 new features that are squared: 'bathrooms\_squared', 'bedrooms\_squared', ''interest\_level\_squared'. Plot a correlation matrix with the new features. Are the new features more correlated with the target than the basic features?
...
5. Now you need to train 3 models: linear regression, decision tree and naive model. We will use them as black boxes without deep understanding.
   1. Results table
      1. Create two empty Pandas DataFrames with columns 'model', 'train', 'test'. Let's call the first one result\_MAE and the second one result\_RMSE. We will fill these tables with the results of the models.
   2. Linear Regression
      ...
      5. Insert your metrics into tables *result\_MAE* and *result\_RMSE* with model name 'linear\_regression'.
   3. Decision Tree
      1. Initialize decision tree regressor from sklearn with fixed random\_state=21.
      ...
      5. Insert your metrics into tables *result\_MAE* and *result\_RMSE* with model name 'decision\_tree'.
   4. Naive Models
      ...
      4. Insert your metrics into tables result\_MAE and result\_RMSE with model names 'naive\_mean' and 'naive\_median'.
   5. Compare the results
      1. Print your final result\_MAE and result\_RMSE tables.
