# Project Mobile 04 — Swift\_Bootcamp

Резюме: в этом проекте ты создашь мобильное приложение на IOS в качестве клиента для реализованного сервера.
...
- В качестве рекомендаций по использованию UI компонентов ты можешь использовать [Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines/components) от Apple.
- Дизайн приложения должен быть интуитивно понятным.

**В проект должны быть добавлены следующие зависимости:**

- CoreData (работа с базой данных);
- Alamofire (сетевое взаимодействие);
- Swinject (внедрение зависимостей);
- Swift Concurrency (асинхронная работа).
...
1. Реализуй слой данных (datasource):
   - Сетевая часть (Retrofit):
     ...
     - Должны быть созданы отдельные модели (Dto), являющиеся наследниками от Decodable.
   ...
   - Часть базы данных (CoreData):
     ...
     - Создай сервис для работы с базой данных, который:
       - осуществляет вызовы запросов и их обработку.
2. Внедри зависимости (DI — Swinject):
   - Опиши создание базы данных.
   - Опиши создание API.
...
2. Внедри зависимости (DI — SwiftInject):
...
1. Реализуй слой пользовательского интерфейса (presentation):
   - Экран существующих игр:
     ...
     - Обновление списка текущих игр:
       - Должен поддерживаться [UIRefreshControl](https://developer.apple.com/documentation/uikit/uirefreshcontrol) для обновления списка существующих игр.
       - При использовании refresh control или при переходе на экран существующих игр на сервер должен отправиться запрос получения списка текущих игр с использованием токена.
   ...
   - Экран текущей игры:
     ...
     - Отображение игрового поля:
       - По нажатии на определенную часть поля на сервер должен отправиться запрос изменения поля текущей игры
       ...
       - Каждую секунду на сервер должен отправляться запрос обновления текущей игры.
     ...
     - Отображение текущего состояния игры:
       ...
       - Победа игрока с UUID — если UUID совпадает с текущим авторизованным пользователем, то должно быть отображение текста «Победа», в противном случае должно быть отображение текста «Поражение» и отсутствовать возможность хода на игровом поле.
