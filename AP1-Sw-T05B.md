# Project Backend 05 — Swift\_Bootcamp

Резюме: в этом проекте ты научишься работать с jwt-авторизацией и расширять возможности веб-приложений на языке **Swift** с использованием **Vapor**.
...
- Добавь библиотеку Vapor для [JWT](https://docs.vapor.codes/security/jwt/).
...
- Создай struct ApiJwtPayload, унаследованный от JwtPayload c переопределенным методом verify и параметрами expiration (срок истечения действия токена) — ExpirationClaim, subject (uuid пользователя) — SubjectClaim.
- Используй стандартные JWT claims для параметров subject и expiration.
- Реализуй класс JwtProvider, у которого есть следующие методы:

  - Для генерации токенов используй JwtKit от Vapor;
...
- Измени метод проверки токена в запросах с методом verify у расширения jwt для Request.
