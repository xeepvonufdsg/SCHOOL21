...
![SimpleNavigator](assets/img_8581.png)
...
![konigsberg-map](assets/img_8052.png)

![simple-konigsberg-map](assets/img_4425.png)
...
![konigsberg-graph](assets/img_7518.png)
...
![not-orient-graph](assets/img_1263.png)
...
![linked-graph](assets/img_9621.png)

- *Disconnected* — the graph has no path between at least two vertices.

![not-linked-graph](assets/img_8012.png)
...
![spanning-tree](assets/img_8676.png)
...
- It is necessary to adapt previously created *self-written* helper classes `Stack` and `Queue` (you can reuse your solution from the *CPP2* project for this) and implement interfaces for them in Swift. These classes must contain the following methods:
...
- Add a new method to the `GraphAlgorithms` class:
  ...
  ```
  struct TsmResult {
      var vertices: [Int]    // an array with the route you are looking for (with the vertex traverse order).
      var distance: Double   // the length of this route
  }
  ```
