...
| Update step 1 | Assignment step 1 | Update step 2 | Assignment step 2 |
| --- | --- | --- | --- |
...
∑Nn=1∥xn−μzn∥2∑n=1N‖xn−μzn‖2\sum\_{n=1}^N \|\mathbf{x}\_n - \boldsymbol{\mu}\_{z\_n}\|^2

where μk=1Nk∑n:zn=kxnμk=1Nk∑n:zn=kxn\boldsymbol{\mu}\_k = \frac{1}{N\_k}\sum\_{n:z\_n=k}\mathbf{x}\_n is the k-cluster center and z∗n=argmink∥xn−μk∥2zn∗=arg⁡mink‖xn−μk‖2z\_n^\* = \arg \min\_k \| \mathbf{x}\_n - \boldsymbol{\mu}\_k \|^2 is the closest cluster of xnxn\mathbf{x}\_n.
...
![Elbow method](assets/img_6321.png)
...
The **"silhouette" coefficient** is computed using the average intra-cluster distance (a) and the average distance to the nearest cluster (b) for each instance i. We define the silhouette coefficient of an instance i as ‘sc(i)=(bi−ai)/max(ai,bi)‘‘sc(i)=(bi−ai)/max(ai,bi)‘` \text{sc}(i) = (b\_i - a\_i) / \max(a\_i, b\_i) `. *Source: [Hidden Markov models for malware classification](https://www.researchgate.net/publication/272017073_Hidden_Markov_models_for_malware_classification)*

![Hidden Markov models for malware classification](assets/img_6022.png)
...
![agglomerative and divisive diagram](assets/img_9862.png)
...
l(θ)=∑NDn=1logp(yn|θ)=∑NDn=1log[∑znp(yn,zn|θ)]l(θ)=∑n=1NDlog⁡p(yn|θ)=∑n=1NDlog⁡[∑znp(yn,zn|θ)]l(\boldsymbol{\theta}) = \sum\_{n=1}^{N\_D} \log p(\mathbf{y}\_n | \boldsymbol{\theta}) = \sum\_{n=1}^{N\_D} \log \left[ \sum\_{\mathbf{z}\_n} p(\mathbf{y}\_n, \mathbf{z}\_n | \boldsymbol{\theta}) \right]

where ‘θ‘‘θ‘`\theta` are Gaussian parameters, ‘zn‘‘zn‘`z\_n` zn are hidden data and ‘yn‘‘yn‘`y\_n` are the visible variables.
...
1. Expectation step: Compute ‘p(yn,zn|θ)‘‘p(yn,zn|θ)‘`p(y\_n, z\_n | \theta)` In other words, does the hidden sample ‘zn‘‘zn‘`z\_n` look like it comes from cluster k?
2. Maximization step: Update the Gaussian parameters (‘θ‘‘θ‘`\theta`) to fit the points assigned to them.
...
7. Try all these algorithms with different sets of features that might be more efficient in clustering. In the checklist will cluster for 3 features — bedrooms, bathrooms and interest\_level.
