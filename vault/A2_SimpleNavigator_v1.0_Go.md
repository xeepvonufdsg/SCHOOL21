...
3. [Chapter III](#chapter-iii) \
   3.1. [Part 1](#part-1-%D0%BE%D0%B1%D1%85%D0%BE%D0%B4-%D0%B3%D1%80%D0%B0%D1%84%D0%B0-%D0%B2-%D0%B3%D0%BB%D1%83%D0%B1%D0%B8%D0%BD%D1%83-%D0%B8-%D0%B2-%D1%88%D0%B8%D1%80%D0%B8%D0%BD%D1%83)\
   3.2. [Part 2](#part-2-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA-%D0%BA%D1%80%D0%B0%D1%82%D1%87%D0%B0%D0%B9%D1%88%D0%B8%D1%85-%D0%BF%D1%83%D1%82%D0%B5%D0%B9-%D0%B2-%D0%B3%D1%80%D0%B0%D1%84%D0%B5)\
   3.3. [Part 3](#part-3-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA-%D0%BC%D0%B8%D0%BD%D0%B8%D0%BC%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D0%B3%D0%BE-%D0%BE%D1%81%D1%82%D0%BE%D0%B2%D0%BD%D0%BE%D0%B3%D0%BE-%D0%B4%D0%B5%D1%80%D0%B5%D0%B2%D0%B0)\
   3.4. [Part 4](#part-4-%D0%B7%D0%B0%D0%B4%D0%B0%D1%87%D0%B0-%D0%BA%D0%BE%D0%BC%D0%BC%D0%B8%D0%B2%D0%BE%D1%8F%D0%B6%D0%B5%D1%80%D0%B0)\
   3.5. [Part 5](#part-5-%D0%BA%D0%BE%D0%BD%D1%81%D0%BE%D0%BB%D1%8C%D0%BD%D1%8B%D0%B9-%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D1%84%D0%B5%D0%B9%D1%81)\
   3.6. [Part 6](#part-6-%D0%B4%D0%BE%D0%BF%D0%BE%D0%BB%D0%BD%D0%B8%D1%82%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE-%D1%81%D1%80%D0%B0%D0%B2%D0%BD%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BC%D0%B5%D1%82%D0%BE%D0%B4%D0%BE%D0%B2-%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D1%8F-%D0%B7%D0%B0%D0%B4%D0%B0%D1%87%D0%B8-%D0%BA%D0%BE%D0%BC%D0%BC%D0%B8%D0%B2%D0%BE%D1%8F%D0%B6%D0%B5%D1%80%D0%B0)
4. [Chapter IV](#chapter-iv)

## Chapter I

![SimpleNavigator](assets/img_9287.png)
...
![konigsberg-map](assets/img_3635.png)

![simple-konigsberg-map](assets/img_5072.png)
...
![konigsberg-graph](assets/img_6870.png)
...
![not-orient-graph](assets/img_4941.png)
...
![linked-graph](assets/img_2438.png)

- *Несвязный* — в графе отсутствует путь хотя бы между двумя его вершинами.

![not-linked-graph](assets/img_8520.png)
...
![tree](assets/img_3596.png)

***Остовное дерево (остов)*** — это подграф данного графа, содержащий все его вершины и являющийся деревом. Рёбра графа, не входящие в остов, называются хордами графа относительно остова.

![spanning-tree](assets/img_2323.png)

### Способы задания графа

Существуют следующие основные способы задания графов:

- *Матрица смежности* — квадратная матрица, размерность которой равна числу вершин в графе, и в которой ‘Aij‘‘Aij‘`A\_{ij}` элемент матрицы содержит в себе информацию о ребре из вершины ‘i‘‘i‘`i` в вершину ‘j‘‘j‘`j`. Возможные значения, которые может принимать ‘Aij‘‘Aij‘`A\_{ij}`:
  ...
  - для невзвешенного ориентированного графа:
    ...
    - 1 — есть дуга (ориентированное ребро), которая направлена из вершины ‘i‘‘i‘`i` в вершину ‘j‘‘j‘`j`
  - для взвешенного ориентированного графа:
    - 0 — дуги между вершинами нет
    - N — есть дуга (ориентированное ребро), которая направлена из вершины ‘i‘‘i‘`i` в вершину ‘j‘‘j‘`j`, и её вес равен N
...
Реализуй библиотеку s21\_graph:

- Библиотека должна быть разработана на языке Go.
- Код библиотеки должен находиться в папке src в ветке develop.
- При написании кода придерживайся Effective Go.
- Оформи решение как статическую библиотеку (s21\_graph).
...
- Сборка программы должна быть настроена с помощью Makefile со стандартным набором целей для GNU-программ: all, clean, test, s21\_graph.
...
- Класс `Graph` должен содержать в себе, по крайней мере, следующие публичные методы:
  - `func LoadGraphFromFile(filename string)` — загрузка графа из файла в формате матрицы смежности.
  - `func ExportGraphToDot(filename string)` — выгрузка графа в файл в формате dot (см. материалы).

Реализуй библиотеку s21\_graph\_algorithms:

- Библиотека должна быть разработана на языке Go.
- Код библиотеки должен находиться в папке src в ветке develop.
- Оформи решение как статическую библиотеку (s21\_graph\_algorithms).
...
- Добавь в Makefile цель s21\_graph\_algorithms.
...
- Класс `GraphAlgorithms` должен содержать в себе, по крайней мере, следующие публичные методы:
  - `func DepthFirstSearch(graph *Graph, start_vertex int)` — *нерекурентный* поиск в глубину в графе от заданной вершины. Функция должна возвращать массив, содержащий в себе обойдённые вершины в порядке их обхода. При реализации этой функции обязательно использовать *самописную* структуру данных **стек**, которую предварительно стоит оформить в виде отдельной статической библиотеки.
  - `BreadthFirstSearch(graph *Graph, start_vertex int)` — поиск в ширину в графе от заданной вершины. Функция должна возвращать массив, содержащий в себе обойдённые вершины в порядке их обхода. При реализации этой функции обязательно использовать *самописную* структуру данных **очередь**, которую предварительно стоит оформить в виде отдельной статической библиотеки.
- Необходимо адаптировать ранее созданные *самописные* вспомогательные классы `Stack` и `Queue` (для этого можно переиспользовать своё решение из проекта *CPP2*) и реализовать для них интерфейсы на Go. Эти классы должны содержать в себе следующие методы:
...
- Добавь в класс `GraphAlgorithms` два новых метода:
  - `func GetShortestPathBetweenVertices(graph *Graph, vertex1 int, vertex2 int)` — поиск кратчайшего пути между двумя вершинами в графе с использованием *алгоритма Дейкстры*. Функция принимает на вход номера двух вершин и возвращает численный результат, равный наименьшему расстоянию между ними.
  - `func GetShortestPathsBetweenAllVertices(graph *Graph)` — поиск кратчайших путей между всеми парами вершин в графе с использованием *алгоритма Флойда-Уоршелла*. В качестве результата функция возвращает матрицу кратчайших путей между всеми вершинами графа.

## Part 3. Поиск минимального остовного дерева

- Добавь в класс `GraphAlgorithms` новый метод:
  - `func GetLeastSpanningTree(graph *Graph)` — поиск наименьшего остовного дерева в графе с помощью *алгоритма Прима*. В качестве результата функция должна возвращать матрицу смежности для минимального остовного дерева.

## Part 4. Задача коммивояжера

- Добавь в класс `GraphAlgorithms` новый метод:
  - `func SolveTravelingSalesmanProblem(graph *Graph)` — решение задачи коммивояжера с помощью *муравьиного алгоритма*. Необходимо найти самый выгодный (короткий) маршрут, проходящий через все вершины графа хотя бы по одному разу с последующим возвратом в исходную вершину. В качестве результата функция должна возвращать структуру `TsmResult`, описанную ниже:
...
- Напиши основную программу, которая представляет из себя консольное приложение для проверки работоспособности реализованных библиотек s21\_graph и s21\_graph\_algorithms.
