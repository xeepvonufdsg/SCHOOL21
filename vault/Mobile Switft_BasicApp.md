...
Разработка и реализация мобильного приложения для операционной системы iOS на языке Swift.
...
- В качестве референса по дизайну ты можешь использовать [SwiftUI tutorial](https://developer.apple.com/tutorials/swiftui).
...
- Swift 5.7;
- iOS SDK V6;
- Cocaopods 1.11.3 (управление зависимостями);
- SwiftUI 4 (создание пользовательского интерфейса);
- Codable (сериализация JSON, стандартная библиотека Swift).

**В проект должны быть добавлены следующие зависимости:**

- Realm 10.35.0 (работа с базой данных);
- Alamofire 5.6.4 (сетевое взаимодействие);
- Swinject 2.8.3 (внедрение зависимостей);
- RxSwift 6.5.0 (асинхронная работа).
...
2. Слой бизнес-логики (domain):
   ...
   - Мапперы должны быть оформлены в виде [методов в расширении](https://docs.swift.org/swift-book/LanguageGuide/Extensions.html).
